...
const UE = require("ue");
const TS_Bullet_1 = require("./TS_Bullet");
const TS_LevelMap_1 = require("./TS_LevelMap");
const puerts_1 = require("puerts");
let ucls = UE.Class.Load('/Game/ThirdPersonCPP/Blueprints/BulletPrefab.BulletPrefab_C');
const BulletMixin = puerts_1.blueprint.tojs(ucls);
const TSBulletWithMixin = puerts_1.blueprint.mixin(BulletMixin, TS_Bullet_1.default);
...
class TS_Player extends UE.unreal_puertsCharacter {
    ...
    ReceiveBeginPlay() {
        ...
        let ucls2 = UE.Class.Load('/Game/ThirdPersonCPP/Blueprints/EnemySpawner.EnemySpawner_C');
        let EnemySpawnerMixin = puerts_1.blueprint.tojs(ucls2);
        let TSLevelMapWithMixin = puerts_1.blueprint.mixin(EnemySpawnerMixin, TS_LevelMap_1.default);
        let levelMap = UE.GameplayStatics.BeginDeferredActorSpawnFromClass(this.world, TSLevelMapWithMixin.StaticClass(), undefined, UE.ESpawnActorCollisionHandlingMethod.AlwaysSpawn);
    ...
    TSShoot() {
        ...
        for (let i = -this.barrageRow; i <= this.barrageRow; ++i) {
            ...
            for (let j = -this.barrageCol; j <= this.barrageCol; ++j) {
                ...
                let bullet = UE.GameplayStatics.BeginDeferredActorSpawnFromClass(this.world, TSBulletWithMixin.StaticClass(), bulletTrans, UE.ESpawnActorCollisionHandlingMethod.AlwaysSpawn);
    ...
    HandleActorBeginOverlap(OverlappedActor, OtherActor) {
